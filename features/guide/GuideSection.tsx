...
export const GuideSection: React.FC = () => {
    ...
    // Spell Slot Button Component
    const SpellSlotButton = ({ level, label, icon: Icon }: { level: number | 'all', label: string, icon?: React.ElementType }) => {
        const isActive = spellLevelFilter === level;
        return (
            <button 
                ...
                className={`
                    ...
                    ${isActive 
                        ? 'bg-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.6)] scale-110 z-10' 
                        : 'bg-void-950/50 border border-white/10 hover:border-purple-400/50 hover:bg-white/5'}
                `}
                title={level === 'all' ? 'Todas' : level === 0 ? 'Truques' : `Nível ${level}`}
            >
                {isActive && (
                    <div className="absolute inset-0 bg-purple-400 rounded-full blur-md opacity-50 animate-pulse"></div>
                )}
                <span className={`relative z-10 font-display font-bold text-lg ${isActive ? 'text-white' : 'text-mystic-500 group-hover:text-purple-300'}`}>
                    {Icon ? <Icon size={18} /> : label}
                </span>
                {level !== 'all' && (
                    <span className="absolute -bottom-6 text-[9px] font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity text-purple-300 whitespace-nowrap pointer-events-none">
                        {level === 0 ? 'Truque' : `Nv. ${level}`}
                    </span>
                )}
            </button>
        );
                    ...
                    <div className="animate-fade-in-up">
                        <div className="glass-panel p-8 rounded-[3rem] bg-void-900/40 border-purple-500/20 relative overflow-hidden mb-8">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-end gap-6 relative z-10">
                                ...
                                {/* Slot Level Selector */}
                                <div className="flex flex-wrap gap-2">
                                    <SpellSlotButton level={'all'} label="T" icon={Infinity} />
                                    <SpellSlotButton level={0} label="0" />
                                    <SpellSlotButton level={1} label="1" />
                                    <SpellSlotButton level={2} label="2" />
                                    <SpellSlotButton level={3} label="3" />
                                    <div className="w-px h-8 bg-white/10 mx-2 self-center hidden sm:block"></div>
                                    {/* Placeholder for higher levels in future */}
                                    <div className="flex gap-2 opacity-30 pointer-events-none grayscale">
                                        <SpellSlotButton level={4} label="4" />
                                        <SpellSlotButton level={5} label="5" />
                                        <SpellSlotButton level={9} label="9" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            {filteredSpells.length === 0 && (
                                <div className="col-span-full text-center py-12 text-mystic-500 font-light">
                                    Nenhuma magia encontrada neste círculo.
                                </div>
                            )}
                        </div>
                    </div>
